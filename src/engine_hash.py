...
class SearchEngineHashes(object):
    ...
    def save_list_of_elements_into_cache(self, collection_name, list_to_save=[], clear_it_before=True):
        if clear_it_before:
            self.cache_for_indexer.delete(
                collection_name
            )
        self.cache_for_indexer.hset(
            collection_name,
            "data",
            list_to_save
        )
