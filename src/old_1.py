...
def get_all_cache_elements_as_list_of_jsons(collection_name, clear_it=True):
    try:
        list_of_elements = eval(
            cache_for_indexer.hget(
                collection_name,
                "data"
            ))
    except:
...
def save_list_of_elements_into_cache(collection_name, list_to_save=[], clear_it_before=True):
    if clear_it_before:
        cache_for_indexer.delete(
            collection_name
        )
    cache_for_indexer.hset(
        collection_name,
        "data",
        list_to_save
    )
...
def main():
    ...
    # ]
    #
    # save_list_of_elements_into_cache("collection1", data, clear_it_before=True)
    # print(get_all_cache_elements_as_list_of_jsons("collection1", clear_it=False))

    start = time.time()
    print_as_list(find_by_component_and_version("openssl", "1.0*"))
